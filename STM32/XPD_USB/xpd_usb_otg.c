...
#define USB_STS_GOUT_NAK                (1 << USB_OTG_GRXSTSP_PKTSTS_Pos)
#define USB_STS_DATA_UPDT               (2 << USB_OTG_GRXSTSP_PKTSTS_Pos)
#define USB_STS_XFER_COMP               (3 << USB_OTG_GRXSTSP_PKTSTS_Pos)
#define USB_STS_SETUP_COMP              (4 << USB_OTG_GRXSTSP_PKTSTS_Pos)
#define USB_STS_SETUP_UPDT              (6 << USB_OTG_GRXSTSP_PKTSTS_Pos)
        ...
        uint8_t ucFIFOx, uint8_t * pucData, uint16_t usLength)
{
    ...
    {
        pxUSB->Inst->DFIFO[ucFIFOx].DR = *((uint32_t *) pucData);
    }
        ...
        uint8_t * pucData, uint16_t usLength)
{
    ...
    for (usWordCount = (usLength + 3) / 4; usWordCount > 0; usWordCount--, pucData += 4)
    {
        *(uint32_t *) pucData = pxUSB->Inst->DFIFO[0].DR;
    }
...
#else
        {
            //USB_REG_BIT(pxUSB,GCCFG,USB_OTG_GCCFG_NOVBUSSENS_Pos) = 1;
						pxUSB->Inst->GCCFG.b.VBDEN=1;
        }
#endif
